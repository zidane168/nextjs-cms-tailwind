...
export default function Github() {
  ...
  const { isValidating, data } = useSWR(
    `http://localhost:3333/githubs?limit=${LIMIT}&page=${pageIndex}`,
    fetcher
  );
  ...
  return (
    <>
      ...
      <div className=" mt-2 mb-4">
        ...
        <p>
          {" "}
          Page: {pageIndex}, showing {LIMIT} record(s){" "}
        </p>
      </div>

      <div className="mt-2 text-center space-x-2">
        <button
          className=" rounded px-6 py-2 bg-theme hover:bg-sky-600 focus:outline-none text-white uppercase"
        ...
        <button
          className="disabled rounded px-6 py-2 bg-theme hover:bg-sky-600 focus:outline-none  text-white uppercase disabled:opacity-50"
          ...
          <div className="grid lg:grid-cols-8 gap-4 sm:grid-cols-4 xs:grid-cols-2">
            {!!data?.length &&
              data?.map((value: any, index: number) => {
                return (
                  <div
                    className="p-2 bg-white shadow-lg rounded "
                    key={value.id}
                  >
                    <div className="w-50 h-50">
                      <Image
                        src={
                          "http://localhost:3333/" +
                          value.githubImages[0].myFiles[0].path
                        }
                        width={"100%"}
                        height={"100%"}
                        alt={value.login}
